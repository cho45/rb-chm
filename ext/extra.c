		...
		VALUE dict)
{
	...
	for (i = 0; i < wlc_count; ++i) {
		...
		if (url && topic) {
			rb_ary_push(
					dict,
					rb_assoc_new(rb_str_new2(url), rb_str_new2(topic))
			);
		}
	...
	int 
chm_search_internal (struct chmFile *chmfile,
		const char *text, int whole_words, 
		int titles_only, VALUE dict)
{
		...
		int titles_only)
{
	...
	dict = rb_ary_new();
	ret = chm_search_internal(chmfile, text, whole_words, titles_only, dict);
	if (ret) {
		return dict;
	} else {
		return Qnil;
